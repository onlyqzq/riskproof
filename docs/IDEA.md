...
> **文档状态：历史产品构想。** 本文包含尚未实现的交互动作和早期 HITL 定位，
> 不应作为当前功能清单。当前事实源请看根目录 README、`SECURITY.md` 和
> [`docs/threat-model.md`](threat-model.md)。产品已经从“风险知情审批插件”演进为
> “确定性工具链执行控制层”，审批只是证据不足时的一种 step-up 动作。
